...
namespace RemoteTech
{
    ...
    public static class RTUtils
    {
        ...
        public static string length(double l)
        {
            double length = Math.Round(l, 0);
            if (length >= 1000 && length < 1000000)
                return Math.Round(length / 1000, 2) + " thousand";
            if (length >= 1000000)
                return Math.Round(length / 1000000, 2) + " million";
            return "" + length;
        }
